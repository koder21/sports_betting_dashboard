...
async def main():
    days_back = 365
    print(f"Starting MLB backfill for last {days_back} days (bypassing last scrape date)...")
    client = ESPNClient()
    async with PlayerStatsScraper.context(client) as scraper:
        async with get_session() as session:
            ...
            print(f"Found {len(game_ids)} MLB games to backfill.")
            total_stats = 0
            for idx, game_id in enumerate(game_ids, 1):
                print(f"[{idx}/{len(game_ids)}] Scraping MLB game {game_id}...")
                try:
            ...
            print(f"MLB backfill complete: {total_stats} player stat records added.")
