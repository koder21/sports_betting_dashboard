...
async def main():
    print("Starting NBA backfill for last 180 days (bypassing last scrape date)...")
    stats_scraper = PlayerStatsScraper(ESPNClient())
    stats_scraper.SPORTS_CONFIG = [("basketball", "nba", "NBA")]
    from backend.db import get_session
    async with get_session() as session:
        # Always fetch all NBA game IDs for the last 180 days
        game_ids = await stats_scraper._get_recent_game_ids(session, "basketball", "nba", 180)
        ...
        for idx, game_id in enumerate(game_ids):
            ...
            try:
                stats_count = await stats_scraper._scrape_game_boxscore(session, game_id, "basketball", "nba", "NBA")
        ...
        print(f"NBA backfill complete: {total_stats} player stat records added.")
