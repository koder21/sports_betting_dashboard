"""
NCAAF-Only Backfill Script
-------------------------
Fetches and upserts NCAAF game and player stats for the last 180 days into Postgres.
...
async def main():
    print("Starting NCAAF backfill for last 180 days (bypassing last scrape date)...")
    stats_scraper = PlayerStatsScraper(ESPNClient())
    stats_scraper.SPORTS_CONFIG = [("football", "college-football", "NCAAF")]
    from backend.db import get_session
    async with get_session() as session:
        # Always fetch all NCAAF game IDs for the last 180 days
        game_ids = await stats_scraper._get_recent_game_ids(session, "football", "college-football", 180)
        print(f"Found {len(game_ids)} NCAAF games to backfill.")
        total_stats = 0
        ...
        for idx, game_id in enumerate(game_ids):
            print(f"[{idx+1}/{len(game_ids)}] Scraping NCAAF game {game_id}...")
            ...
            try:
                stats_count = await stats_scraper._scrape_game_boxscore(session, game_id, "football", "college-football", "NCAAF")
        ...
        print(f"NCAAF backfill complete: {total_stats} player stat records added.")
