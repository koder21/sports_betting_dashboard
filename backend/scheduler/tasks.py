            ...
            for sport_type, league, sport_name in SPORTS_CONFIG:
                ...
                for data, finals_only in [(today_data, False), (yesterday_data, True)]:
                    ...
                    for event in events:
                        ...
                        home_score = int(home_team.get("score", 0)) if home_team.get("score", 0) is not None else None
                        away_score = int(away_team.get("score", 0)) if away_team.get("score", 0) is not None else None
        ...
        try:
            ...
            async with self.session_factory() as session:
                result = await session.execute(
                    select(GameResult).where(
                        not_(
                            exists().where(PlayerStats.game_id == GameResult.game_id)
                        )
                    )
                )
                ...
                for gid in missing_game_ids:
                    ...
                    for model in [Game, GameResult, GameLive, GameUpcoming]:
                        result = await session.execute(select(model).where(model.game_id == gid))
                        game = result.scalar()
                        if game:
                            break
                    if not game or not getattr(game, "sport", None):
                        logger.warning(f"[Backfill] Could not find sport for game {gid}, skipping.")
                        continue
                    sport_upper = (game.sport or "").upper()
                    if sport_upper not in sport_league_map:
                        logger.warning(f"[Backfill] Unknown sport: {game.sport} for game {gid}, skipping.")
                        continue
                    sport_type, league = sport_league_map[sport_upper]
                    ...
                    if missing:
                        ...
                        competitions = summary.get('competitions', [{}])[0] if summary else {}
                        ...
                        await session.execute(
                            pg_insert(Game).values(
                                ...
                                home_team_id=f"NBA-{home_team.get('id')}" if home_team.get('id') else None,
                                away_team_id=f"NBA-{away_team.get('id')}" if away_team.get('id') else None,
                        ...
                        games_results_upsert_dict = {
                            ...
                            "home_team_id": f"NBA-{home_team.get('id')}" if home_team.get('id') else None,
                            "away_team_id": f"NBA-{away_team.get('id')}" if away_team.get('id') else None,
                            "home_team": home_team.get('displayName'),  # actual DB column
                        ...
                        try:
                            ...
                            logger.error(f"[Backfill] Commit failed after upsert_game_result for game {gid}: {commit_e}", exc_info=True)
                    ...
                    try:
                        ...
                        if hasattr(stats_scraper, '_scrape_game_boxscore'):
                            await stats_scraper._scrape_game_boxscore(session, gid, sport_type, league, sport_upper)
                        else:
                            logger.error(f"[Backfill] PlayerStatsScraper missing _scrape_game_boxscore method for game {gid}")
                        await session.commit()
                    except Exception as e:
                        if 'greenlet_spawn' in str(e) or "can't call await_only()" in str(e):
                            logger.error(f"[Backfill] Async context error for game {gid}: {e}. Skipping.")
                            continue
