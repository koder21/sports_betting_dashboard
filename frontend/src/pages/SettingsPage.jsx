...
function SettingsPage() {
  ...
  const handleRunAll = useCallback(async () => {
    ...
    try {
      const res = await api.post('/api/scrape/run-all');
      setRefreshResult({ status: res.data.status, results: res.data.results });
    } catch (e) {
      ...
      <div className="settings-container">
        <div className="settings-section">
          ...
          <div className="setting-item">
            <label htmlFor="timezone-select">
              ...
              <p className="setting-description">
                Select your timezone. All times displayed on the site will be converted to this
                timezone. This setting only affects your display—it does not change any data in the
                database.
              </p>
            </label>
          ...
          <div className="setting-item">
            <label htmlFor="odds-format-select">
              ...
              <p className="setting-description">
                Choose how odds are displayed across the site. This setting only affects your
                display—all data in the database remains in American format.
              </p>
            </label>
        ...
        <div className="settings-section">
          ...
          <div className="setting-item">
            ...
            <p className="setting-description">
              Scrape all sports, update live scores, grade pending bets, and backfill player stats.
            ...
            <button
              ...
              disabled={refreshing}
              className="save-button"
              style={{ background: refreshing ? '#555' : '#2563eb' }}
            >
              {refreshing ? '⏳ Running...' : '🔄 Run Full Refresh'}
            </button>
            {refreshResult && (
              <div style={{ marginTop: '1rem', fontSize: '0.85rem' }}>
                <strong>Result: {refreshResult.status}</strong>
                {refreshResult.results && (
                  <ul style={{ marginTop: '0.5rem' }}>
                    {Object.entries(refreshResult.results).map(([task, res]) => (
                      <li key={task}>
                        {task}: {res}
                      </li>
                    ))}
                ...
                {refreshResult.message && <p>{refreshResult.message}</p>}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="settings-section info-section">
          ...
          <div className="info-content">
            <p>
              <strong>Frontend-Only:</strong> All display preferences are applied only in your
              browser. Your backend data and database are not affected.
            </p>
            <p>
              <strong>Auto-Apply:</strong> Once set, your preferences are automatically applied to
              all times displayed across the site:
            </p>
            <ul>
            ...
            <p>
              <strong>Storage:</strong> Your timezone preference is saved in your browser's local
              storage and persists across sessions.
            </p>
          </div>
        </div>
      </div>
