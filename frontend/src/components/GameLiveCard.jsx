...
const GameLiveCard = ({ gameData }) => {
  // Parse the metadata
  const metadata =
    typeof gameData.metadata === 'string' ? JSON.parse(gameData.metadata) : gameData.metadata;
  ...
  const { home_score, away_score, sport, status, period, clock } = metadata;
  ...
  return (
    <div className="game-live-card">
      ...
      <div className="live-footer">
        ...
        {status && !clock && <div className="game-status">{status}</div>}
      </div>
    </div>
  );
};
