...
const BetWinCard = ({ betData }) => {
  // Parse the metadata
  const metadata =
    typeof betData.metadata === 'string' ? JSON.parse(betData.metadata) : betData.metadata;

  const { selection, odds, stake, profit, bet_type, sport, game_info, status, legs } = metadata;
  ...
  return (
    <div className={`bet-win-card ${isWon ? 'won' : 'lost'}`}>
      <div className="bet-header">
        ...
        <div className={`status-badge ${isWon ? 'won' : 'lost'}`}>{isWon ? '✓ WON' : '✗ LOST'}</div>
      </div>
      ...
      <div className="bet-content">
        ...
        {isFinished && betData.final_score && (
          <div className="bet-final-score">
            ...
            <span className="final-score-value">
              {betData.final_score.replace('Final:', '').trim()}
            </span>
          </div>
        )}
        ...
        {game_info && <div className="game-context">{game_info}</div>}
          ...
          <div className="parlay-legs">
            ...
            {legs.map((leg, idx) => (
              <div key={idx} className="leg-item">
                <span className={`leg-status ${leg.status}`}>
                  {leg.status === 'won' ? '✓' : '✗'}
                </span>
