...
const BetStats = React.memo(({ stats }) => {
  ...
  const statItems = [
    ...
    { label: 'Win Rate', value: formatPercentage(winRate, 1), className: winRate >= 50 ? 'win' : 'loss' },
    { label: 'Total Staked', value: formatCurrency(totalStake, false, true), className: 'neutral' },
    { label: 'P&L', value: formatCurrency(totalPnl, true, true), className: totalPnl >= 0 ? 'win' : 'loss' },
    { label: 'ROI', value: `${roi >= 0 ? '+' : ''}${formatPercentage(roi, 1)}`, className: roi >= 0 ? 'win' : 'loss' },
  ];
